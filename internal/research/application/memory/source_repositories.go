...
func (repository sourceRepository) SetKind(ctx context.Context, id research.SourceID, kind research.SourceKind) error {
	const operation = "set memory source kind"
	if err := contextError(operation, ctx); err != nil {
		return err
	}
	if err := id.Validate(); err != nil {
