...
func disableEcho(input *os.File) (func() error, error) {
	...
	if err := ioctl(input.Fd(), syscall.TIOCGETA, &original); err != nil {
		return nil, err
	}
	updated := original
	updated.Lflag &^= syscall.ECHO
	if err := ioctl(input.Fd(), syscall.TIOCSETA, &updated); err != nil {
		return nil, err
	}
	return func() error { return ioctl(input.Fd(), syscall.TIOCSETA, &original) }, nil
}
