...
func (service *Service) executeStreak(ctx context.Context, command Command) (result Result, err error) {
	...
	if store.Streaks() == nil {
		return Result{}, errors.New("streak service is unavailable")
	}
	streak, err := store.Streaks().Show(ctx)
